...
from refs.models import User, RefLevel, UserGraph
from typing import (
    List,
    Dict,
    Tuple,
...
class RefService:
    def __init__(self, refs_user_data_path: str) -> None:
        ...
        self._user_graph = self._load_user_graph(refs_user_data_path)

        self._pk2user = {user.pk: user 
                         for user in self._iter_over_subtree(self._user_graph.root)}
    ...
    def get_user(self, pk: str) -> User:
        user = self._pk2user.get(pk)

        if user is None:
            raise ValueError(f"There is not user with pk '{pk}'")

        return user

    def get_users(self) -> List[User]:
        return list(self._pk2user.values())

    def _load_user_graph(self, data_path: str) -> UserGraph:
